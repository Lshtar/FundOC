...
NSString * const HXFNetworkDomain = @"http://101.200.139.156";
...
NSString * const HXFNetworkDomain = @"http://www.xiaoban.mobi";
...
+ (instancetype)requestWithSuccessBlock:(HXFRequestSuccessBlock)successBlock failureBlock:(HXFRequestFailureBlock)failureBlock {
    return [[[self class] alloc] initWithSuccessBlock:successBlock failureBlock:failureBlock];
}

- (void)startCompletionBlockWithSuccess:(HXFRequestSuccessBlock)success failure:(HXFRequestFailureBlock)failure {
...
- (void)startRequest {
    _networkIsError = [[HXFReachability reachabilityWithHostName:@"www.baidu.com"] currentReachabilityStatus] == NotReachable ? YES : NO;
    if (_networkIsError) {
...
- (void)constructURL {
    _url = [NSString stringWithFormat:@"%@%@", HXFNetworkDomain, [self requestURLPath]];
}
...
- (NSURLSessionTask *)sessionTaskForError:(NSError *__autoreleasing *)error {
    HXFRequestMethod method = [self requestMethod];
...
- (AFHTTPRequestSerializer *)requestSerializer {
    ...
    switch ([self requestSerializerType]) {
        case HXFRequestSerializerTypeHTTP: {
            requestSerializer = [AFHTTPRequestSerializer serializer];
        } break;
        case HXFRequestSerializerTypeJSON: {
...
- (HXFRequestMethod)requestMethod {
    return HXFRequestMethodGET;
}

- (HXFRequestSerializerType)requestSerializerType {
    return HXFRequestSerializerTypeHTTP;
}

- (NSDictionary *)requestArguments {
    NSAssert([self isMemberOfClass:[HXFNetWorkTool class]], @"子类必须实现requestArguments");
    return nil;
}

- (NSString *)requestURLPath {
    NSAssert([self isMemberOfClass:[HXFNetWorkTool class]], @"子类必须实现requestURLPath");
    return nil;
}
...
- (void)handleData:(id)data errCode:(NSInteger)errCode {
    NSAssert([self isMemberOfClass:[HXFNetWorkTool class]], @"子类必须实现handleData:data errCode:errCode");
}
